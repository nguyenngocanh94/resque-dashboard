...
class ResqueStat
{
    ...
    public function getWorkers()
    {
        ...
        for ($i = 0, $total = count($result), $j = 0; $i < $total; $i += 3) {
            $this->workers[$j]['start'] = new \DateTime(time());
    ...
    public function getQueues($fields = array(), $queues = array())
    {
        ...
        if (in_array('totaljobs', $fields)) {
            ...
            Service::Mongo()
                ->selectCollection(Service::$settings['Mongo']['database'], 'map_reduce_stats')
                ->updateOne(
                    array('_id' => 'queue_stats'),
                    array('$set' => array('date' => $now, 'stats' => $results)),
                    array('upsert' => true)
                );
    ...
    public function getJobs($options = array())
    {
        ...
        if ($options['type'] == 'count') {
            return $jobsCollection->countDocuments($conditions);
        }

        $jobsCursor = $jobsCollection->find($conditions, $options);
...
//        }
    ...
    public function getJobsCount($options = array())
    {
        ...
        if (!empty($options['date_after'])) {
            $conditions['t']['$gte'] = new UTCDateTime($options['date_after']);
        }

        if (!empty($options['date_before'])) {
            $conditions['t']['$lt'] = new UTCDateTime($options['date_before']);
        }
    ...
    {
        $mapReduceStats = Service::Mongo()->selectDB(Service::$settings['Mongo']['database'])->{'map_reduce_stats'};
        ...
        $map = new Javascript("function() {emit(this.d.args.payload.class, 1); }");
        ...
        $options = ['value'=>-1, 'limit'=>$limit];
        $cursor = Service::Mongo()->selectCollection(Service::$settings['Mongo']['database'], 'jobs_repartition_stats')->find();

        $stats->total = Service::Mongo()->selectCollection(Service::$settings['Mongo']['database'], 'got_events')->countDocuments();
        $stats->stats = array();
        foreach ($cursor as $c) {
